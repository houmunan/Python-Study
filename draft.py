...
二进制直接转化到10进制（把10进制考虑成释义），比如说 011=3， 010=2， 111=7，0111 1111=127，1111 1111=255，是这样，但这样没法表示负数。所以现在无论是几个bit组成的二进制数，都把做左边一个保留来表达正负，0为正，1为负；因为牵扯到0000 0000是0但1111 1111不能是-0只能是-1。比如1111 1111按二进制算原本是255，但现在用1111 1111代表一个负数，这个负数覆盖了255这个原本含义，于是1111 1111从此不再是255了；换算方法就是Two's complement。Two's complement是对“一个负bit数字代表的是哪个负的正数？”这个问题的解释，不是一种运算. 所以叫“Two's-complement representation”而不是“Two's-complement transformation”。具体法则如下：
